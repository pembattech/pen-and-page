...
class Blog(models.Model):
    title = models.CharField(max_length=250)  # A field to store the title of the blog post.
    author = models.ForeignKey(User, on_delete=models.CASCADE, null= True)  # ForeignKey relationship with User model
    short_description = models.TextField(null=True)
    content = RichTextUploadingField()  # A field to store the main content of the blog post.
    ...
    cover_image = models.ImageField(upload_to="static/public/cover_image/")  # A field to store the image associated with the blog post.
